...
class Ajax extends Backend
{
    ...
    /** 获取栏目类型 模板
     * @Author: hejiaz
     * @Date: 2021-01-12 14:25:08
     */
    public function cmstemp()
    {
        $type = $this->request->request('type/s', '', 'trim');
        $cmstemp = config('cmstemp');

        if($type){
            foreach ($cmstemp[$type] as $k => $val) {
                $cms_temp_data[] = [
                    'value' => $val,
                    'name' => $val,
                ];
